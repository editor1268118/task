<x-app-layout>
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h2 class="h4 mb-1">Notification Center</h2>
                <p class="text-muted mb-0">Your complete task, query, finance, and approval alert history.</p>
            </div>
        ...
        <div class="card-body p-0">
            @forelse($notifications as $notification)
                @php
                    ...
                    $url = $data['url'] ?? route('notifications.read', $notification->id);
                ...
                <a href="{{ route('notifications.read', $notification->id) }}" class="d-flex gap-3 p-3 border-bottom text-decoration-none text-dark {{ $notification->read_at ? '' : 'bg-light' }}">
                    <div class="rounded-circle d-flex align-items-center justify-content-center {{ $notification->read_at ? 'bg-secondary-subtle text-secondary' : 'bg-primary-subtle text-primary' }}" style="width: 42px; height: 42px;">
