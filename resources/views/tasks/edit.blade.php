...
@section('page-header', 'Edit Task: ' . $task->task_no)
...
@section('content')
<form action="{{ route('tasks.update', $task) }}" method="POST" data-loading>
    @csrf
    @method('PUT')
            ...
            <div class="card border-0 shadow-sm mb-4 h-100">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="8">{{ old('description', $task->description) }}</textarea>
                        @error('description')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="mb-3">
                        <label for="remarks" class="form-label">Remarks / Additional Notes</label>
                        <textarea class="form-control @error('remarks') is-invalid @enderror" id="remarks" name="remarks" rows="2">{{ old('remarks', $task->remarks) }}</textarea>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('customer_id') is-invalid @enderror" id="customer_id" name="customer_id">
                            ...
                            @foreach($customers as $customer)
                                <option value="{{ $customer->id }}" data-name="{{ $customer->company_name ?: $customer->contact_person }}" data-mobile="{{ $customer->mobile }}" {{ old('customer_id', $task->customer_id) == $customer->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('client_name') is-invalid @enderror" id="client_name" name="client_name" value="{{ old('client_name', $task->client_name) }}" placeholder="Client name">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control @error('client_contact') is-invalid @enderror" id="client_contact" name="client_contact" value="{{ old('client_contact', $task->client_contact) }}" placeholder="Phone / Email">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('additional_info') is-invalid @enderror" id="additional_info" name="additional_info" rows="2" placeholder="Any additional details...">{{ old('additional_info', $task->additional_info) }}</textarea>
            ...
            <div class="card border-0 shadow-sm mb-4">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control bg-light fw-bold text-primary" value="{{ $task->task_no }}" readonly>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('task_type_id') is-invalid @enderror" id="task_type_id" name="task_type_id" required>
                            ...
                            @foreach($taskTypes as $type)
                                <option value="{{ $type->id }}" {{ old('task_type_id', $task->task_type_id) == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('assigned_to') is-invalid @enderror" id="assigned_to" name="assigned_to" required>
                            ...
                            @foreach($assignees as $assignee)
                                <option value="{{ $assignee->id }}" data-department-id="{{ $assignee->department_id }}" {{ old('assigned_to', $task->assigned_to) == $assignee->id ? 'selected' : '' }}>{{ $assignee->name }}</option>
                            @endforeach
                        </select>
                        @if(auth()->user()->hasRole('super-admin'))
                    ...
                    <div class="row">
                        ...
                        <div class="col-6 mb-3">
                            ...
                            <input type="number" step="0.5" class="form-control @error('actual_hours') is-invalid @enderror" id="actual_hours" name="actual_hours" value="{{ old('actual_hours', $task->actual_hours) }}">
